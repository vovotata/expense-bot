...
func (h *Handler) HandleCallback(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	case strings.HasPrefix(data, "adm:"):
...
func (h *Handler) handleTypeCallback(b *gotgbot.Bot, ctx *ext.Context, expType string) error {
	...
	_, _, _ = ctx.EffectiveMessage.EditText(b,
		fmt.Sprintf("Тип расходника: <b>%s</b>", state.ExpenseType.Label()),
...
func (h *Handler) handlePaymentCallback(b *gotgbot.Bot, ctx *ext.Context, payment string) error {
	...
	_, _, _ = ctx.EffectiveMessage.EditText(b,
		fmt.Sprintf("Способ оплаты: <b>%s</b>", state.PaymentMethod.Label()),
		&gotgbot.EditMessageTextOpts{ParseMode: "HTML"},
	)
...
func (h *Handler) handleConfirmCallback(b *gotgbot.Bot, ctx *ext.Context, action string) error {
	...
	case "edit":
		...
		_, _, _ = ctx.EffectiveMessage.EditText(b, "Что хотите изменить?", &gotgbot.EditMessageTextOpts{
			ReplyMarkup: kb,
		})
		return nil
	case "cancel":
		_ = h.fsm.Delete(dbCtx, userID)
		_, _, _ = ctx.EffectiveMessage.EditText(b, "❌ Заявка отменена.", nil)
		return nil
	}
	return nil
}
...
func (h *Handler) submitRequest(b *gotgbot.Bot, ctx *ext.Context, state *fsm.WizardState) error {
	...
	if err != nil {
		...
		_, _, _ = ctx.EffectiveMessage.EditText(b, "❌ Ошибка при сохранении заявки. Попробуйте /start заново.", nil)
		return nil
	}
...
func (h *Handler) notifyAdmin(b *gotgbot.Bot, state *fsm.WizardState, user *gotgbot.User, requestID string) error {
	...
	kb := keyboards.AdminRequestKeyboard(requestID[:8])
	...
	// Save tg_message_id for future updates
	if msg != nil {
		reqID, _ := uuid.Parse(requestID)
		_, _ = h.store.UpdateRequestStatus(context.Background(), reqID, domain.StatusPending)
	}
...
func (h *Handler) handleAdminCallback(b *gotgbot.Bot, ctx *ext.Context, data string) error {
	// Format: adm:ACTION:REQUEST_SHORT_ID
	...
	// Find request by short ID prefix
	// For now, update the message text to reflect status change
	...
	slog.Info("admin status change", "action", action, "request_short_id", shortID)
	return nil
}

func (h *Handler) handleDeleteMailCallback(b *gotgbot.Bot, ctx *ext.Context, idStr string) error {
	// Will be implemented in email-watcher stage
